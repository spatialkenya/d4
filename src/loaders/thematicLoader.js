...
const thematicLoader = async (config) => {
  ...
  if (
    !legendSet &&
    dataItem.legendSet &&
    (config.method === undefined || config.method === CLASSIFICATION_PREDEFINED)
  ) {
  ...
  if (!isSingleColor) {
    legendItems = legendSet
      ...
      : getAutomaticLegendItems(orderedValues, method, classes, colorScale);
  }
  ...
  if (dimensions && dimensions.length) {
    legend.filters = dimensions.map(
      (d) =>
        `${names[d.dimension]}: ${d.items.map((i) => names[i.id]).join(", ")}`
    );
  }
